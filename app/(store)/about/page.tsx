...
export default function AboutPage() {
  ...
  // CMS-driven content
  const heroTitle = getSetting('about_hero_title') || 'More Than Just A Brand';
  const heroSubtitle = getSetting('about_hero_subtitle') || 'From Accra to your doorstep — quality dresses, electronics, bags, shoes and more at prices that make sense.';
  ...
  const mission1Title = getSetting('about_mission1_title') || 'Everything in One Place';
  const mission1Content = getSetting('about_mission1_content') || 'From fashion to electronics, bags to shoes — we aim to be the only store you need. Our catalogue is constantly expanding with new arrivals sourced from trusted local and international suppliers.';
  const mission2Title = getSetting('about_mission2_title') || 'Empowering Resellers';
  const mission2Content = getSetting('about_mission2_content') || 'We support small businesses and resellers with competitive bulk pricing. Many of our products are available at wholesale rates, helping entrepreneurs across Ghana grow their own ventures.';
  ...
  const ctaTitle = getSetting('about_cta_title') || 'Ready to shop smarter?';
  const ctaSubtitle = getSetting('about_cta_subtitle') || 'Browse our collection of dresses, electronics, bags, shoes and more. New stock arrives weekly.';
