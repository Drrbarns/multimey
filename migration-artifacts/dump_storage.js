#!/usr/bin/env node
/**
 * Download storage objects for MultiMey Supplies into migration-artifacts/storage.
...
const BUCKETS = ["avatars", "blog", "media", "products", "reviews", "site-assets"];
...
async function main() {
  ...
  for (const b of BUCKETS) {
    ...
    summary[b] = await walk(b);
    console.log(`  total files: ${summary[b]}`);
  }
  fs.writeFileSync(
    path.join(__dirname, "dumps", "_storage_summary.json"),
    JSON.stringify({ dumpedAt: new Date().toISOString(), buckets: summary }, null, 2)
  );
